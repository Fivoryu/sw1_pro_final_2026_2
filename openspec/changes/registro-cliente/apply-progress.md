...
- Current boundary: slice 2 of 4, T2 migration only, on child branch `feat/registro-cliente/t2-migracion` based on the T1 slice. Parent prompt explicitly authorizes branch commits and pushes for this change, overriding the older task note that prohibited them until notice.
- T3 and T4+T5 remain separate child slices and will not be mixed into this commit.
...
- After T2: implementation tasks T1 and T2 are complete; T3–T5 remain unchecked; parent-owned lifecycle rows remain unchanged; apply remains in progress and routes to the next implementation slice, not verify or parent lifecycle yet.
